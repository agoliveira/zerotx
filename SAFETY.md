...
The audio package looks up sample files at `<sounds-dir>/<lang>/<name>.<ext>`
with a fallback to `<sounds-dir>/<name>.<ext>`. The actual sample files are
generated from a YAML dictionary (`sounds/dictionary.yml`) by
`scripts/build-sounds.sh`, which calls Microsoft Edge TTS to synthesise the
spoken text per language.

The generated audio is **not committed to the repo** — it's regenerated by
contributors on demand. The dictionary, build script, and any hand-recorded
overrides are committed; the per-language MP3 directories are gitignored.

**Hand-recorded overrides** at `sounds/overrides/<lang>/<name>.{mp3,wav}` are
...
The audio bank is **purely advisory**. Missing or corrupt audio files
produce a one-time log warning per file and silence for that event;
nothing in the flight-critical path depends on audio playback succeeding.
The choice to ship Edge TTS-generated audio is documented in
`sounds/ATTRIBUTION.md` along with regeneration paths for contributors who
need a different licensing posture.
